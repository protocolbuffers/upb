...
//
// std::string MakeReadableFakeData(size_t size) {
...
// }
//
// class FlatDataAndMaybeStream {
//  public:
//   FlatDataAndMaybeStream(const std::string& data,
...
//   }
//
//   absl::string_view flat_data() const { return flat_data_; }
//   upb_ZeroCopyInputStream* seam_stream() const { return seam_stream_.get(); }
//
//  private:
...
//       VectorOf(InRange(1, max_data_size))  // chunks
//   );
// }
//
...
//   std::string test_data = MakeReadableFakeData(script.data_size);
//   absl::string_view test_data_view(test_data);
//   FlatDataAndMaybeStream input(test_data, script.chunks);
//   upb_EpsCopyInputStream eps;
//
//   const char* ptr = upb_EpsCopyInputStream_Init(&eps, input.flat_data().data(),
//                                                 input.flat_data().size(),
//                                                 input.seam_stream());
//
//   int end_ofs = eps.end - ptr;
//   int overrun = ptr - eps.end;
//   if (overrun >= 0) ptr = upb_EpsCopyInputStream_Skip(&eps, overrun, &end_ofs);
//   if (ptr) EXPECT_EQ(end_ofs, eps.end - ptr);
//
//   size_t ofs = 0;
//
//   while (ptr) {
//     EXPECT_LT(ptr, eps.end);
//     size_t have = eps.end - ptr;
//     EXPECT_LT(ofs, test_data.size());
//     EXPECT_LE(ofs + have, test_data.size());
//     DBG("ofs=%d, have=%d\n", (int)ofs, (int)have);
//     size_t remaining = test_data.size() - ofs;
//     EXPECT_LE(have, remaining);
//     size_t must_be_valid = std::min(remaining, have + kSlopBytes);
//     EXPECT_LE(ofs + must_be_valid, test_data.size());
//     EXPECT_EQ(absl::string_view(ptr, must_be_valid),
//               test_data_view.substr(ofs, must_be_valid));
//     // Test that we can read these bytes without error.
//     memcpy(&tmp_buf[0], eps.end, kSlopBytes);
//     ofs += have;
//     ptr = upb_EpsCopyInputStream_Next(&eps);
//   }
//
//   EXPECT_EQ(ofs, test_data.size());
// }
//
// // Test with:
...
//   TestAgainstFakeStream({512, {1}});
// }
//
// TEST(EpsCopyFuzzTest, Foo) { TestAgainstFakeStream({154, {}}); }
//
// TEST(EpsCopyFuzzTest, Empty) { TestAgainstFakeStream({0, {}}); }
//
// TEST(EpsCopyFuzzTest, Short) { TestAgainstFakeStream({3, {}}); }
//
// #if 0
//
// class EpsStream {
